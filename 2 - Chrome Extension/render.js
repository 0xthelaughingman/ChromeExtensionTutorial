var inject = '('+function() {
	...
	let Animator = {
		...
		tfjs_draw_counter: 0,
		video_on: false,
		draw_type: "blur",    //  This should have a function defined that's called in get_canvas_stream that reads from the Extension's persistent data,
	...
	function add_dynamic_elements(constraints, fallback_constraints)
	{
		...
		*/
		...
		{
			invisible_video.height = limit_tfjs.height.max
			invisible_video.width = limit_tfjs.width.max
			invisible_video.style.right = (2 * limit_tfjs.width.max)
			...
			tfjs_feed.height = limit_tfjs.height.max
			tfjs_feed.width = limit_tfjs.width.max
			tfjs_feed.style.position= "absolute"
			tfjs_feed.style.right = (4 * limit_tfjs.width.max)
